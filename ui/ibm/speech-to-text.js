  ...
  function micON() { // When the microphone button is clicked
    if (recording === false) {
      if (records === 0) { // The first time the mic is clicked - inform user
        Api.setWatsonPayload({output: {text: ['Please accept microphone request so I can further assist you.'], ref: 'STT'}}); // Dialog box output to let the user know we're recording
        records++;
      } else {
    ...
    fetch('/api/speech-to-text/token')        // Fetch authorization token for Watson Speech-To-Text
      ...
      .then(function(token) {                 // Pass token to Watson Speech-To-Text service
        stream = WatsonSpeech.SpeechToText.recognizeMicrophone({
          ...
          inactivity_timeout: 999999,              // Number of seconds to wait before closing input stream
        ...
        stream.promise()                                // Once all data has been processed...
          .then(function(data) {                        // ...put all of it into a single array
            ...
            if (data.length !== 0) {                    // If data is not empty (the user said something)
              ...
              if ((dialogue.alternatives[0].transcript !== '') && (dialogue.final === true) && ( dialogue.alternatives[0].transcript.includes("V") || dialogue.alternatives[0].transcript.includes("Vee") || dialogue.alternatives[0].transcript.includes("Ve") || dialogue.alternatives[0].transcript.includes("Ivy"))) { // Another check to verify that the transcript is not empty and that this is the final dialog
                Conversation.sendMessage();             // Send the message to Watson Conversation
                  var watsonResponse = Api.getWatsonPayload()
