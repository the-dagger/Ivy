...
var ConversationResponse = (function() {
  ...
  function setupResponseFunctions() {
    responseFunctions = {
      ...
      weather: {
        city: function(value){ console.log("Weather called");Panel.fetchWeather(value);}
      },
        stocks: {
            stocks: function(value){ console.log("Stocks called");Panel.fetchStocks(value);}
        },
    movies: {
        movies: function(value){ console.log("Movie called");Panel.fetchMovies(value);}
      },
      off_topic: {
        amenity: {
          gas: function() { Panel.mapGas(); },
  ...
  function responseHandler(data) {
    if (data && data.intents && data.entities && !data.output.error) {
      ...
      if (data.context.callRetrieveAndRank && !data.output.text) {
        // TODO add EIR link
        data.output.text = ['I am not able to answer that. Please try again.'];
